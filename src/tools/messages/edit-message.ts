...
/**
 * Discord Embed の型定義（send-messageと共通）
 */
...
export const toolDefinition: ToolDefinition = {
  name: 'edit_message',
  description: 'Discordの既存メッセージを編集します（自分が送信したメッセージのみ）',
  inputSchema: {
    ...
    properties: {
      ...
      content: {
        type: 'string',
        description: '新しいメッセージの内容（最大2000文字）'
      },
      embeds: {
        type: 'array',
...
export type EditMessageOutput = z.infer<typeof EditMessageOutputSchema>;
...
export async function editMessage(
  ...
  try {
    ...
    const message = await discordClient.editMessage(input.channelId, input.messageId, messageData);
    ...
    const errorMessage = error instanceof Error ? error.message : 'メッセージの編集中に不明なエラーが発生しました';
    throw new Error(`メッセージの編集に失敗しました: ${errorMessage}`);
  }
}